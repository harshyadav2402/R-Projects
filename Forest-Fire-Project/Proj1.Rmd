...
ggplot(fire, aes( x = wind , y = temp, fill = FFMC ) ) +  # print bar chart
...
plot(fire$wind, fire$temp, pch = 16, col = mycolors[fire$FFMC_CAT],
     main="FFMC variation with Temp and Wind",xlab="Temperature", ylab="Wind")
legend("topright", legend=levels(fire$FFMC_CAT), pch=16, cex=0.6, col=unique(mycolors))
```

##This plot shows that for lower values of wind and higher values of temperature the FFMC values are very high and thus there is very high chances of fire when the wind speed is low and the temperature is high.
...
```{r}
#cor(fire$ISI,fire$wind)
#cor(fire$ISI, fire$FFMC)
...
```{r}
fire_rf <- read.csv("forestfires.csv", header=TRUE, stringsAsFactors=FALSE)
```
